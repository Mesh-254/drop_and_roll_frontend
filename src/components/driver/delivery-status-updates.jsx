...
export function DeliveryStatusUpdates({
  ...
  const checkAllSelectedImmutable = useCallback(async () => {
    ...
    try {
      ...
      console.error(
        "[DeliveryStatusUpdates] Batch immutable check error:",
        error
      );
  ...
  const handleBulkStatusUpdate = useCallback(
    async (newStatus) => {
      ...
      if (updatableJobs.length === 0) {
        toast.error(
          "No jobs can be updated—all selected are delivered with POD submitted."
        );
        return;
      }
      ...
      try {
        if (skippedCount > 0) {
          toast.warning(
            `Skipping ${skippedCount} delivered jobs with POD. Updating ${updatableJobs.length}.`
          );
        }
        const response = await driverApi.bulkUpdateStatus(
          updatableJobs,
          newStatus
        );
        toast.success(
        ...
        console.error(
          "[DeliveryStatusUpdates] Bulk status update error:",
          error
        );
      }
    },
    [
      checkAllSelectedImmutable,
      updatableJobs,
      skippedCount,
      selectedJobs.length,
      fetchJobs,
  ...
  const handleSingleStatusUpdate = useCallback(
    async (jobId, newStatus) => {
      ...
      if (check.immutable) {
        toast.error(
          check.reason || "Job is delivered with POD submitted—cannot update."
        );
        return;
      }
                        ...
                        {(job.notes || job.quote?.meta?.special_instructions)
                          ...
                          <button
                            onClick={(e) => {
                              ...
                              toast(
                                job.notes ||
                                  job.quote?.meta?.special_instructions,
                                { duration: 5000 }
                              );
                            }}
