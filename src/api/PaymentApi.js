...
export class PaymentApi extends ApiBase {
  ...
  async initiateTransaction(txId, includeAuth = true, guestEmail = "") {
    try {
      ...
      const response = await this.request(
        `/api/payments/transactions/${txId}/initiate/`,
        {
          method: "POST",
  ...
  async captureTransaction(txId, includeAuth = true, guestEmail = "") {
    try {
      ...
      const response = await this.request(
        `/api/payments/transactions/${txId}/capture/`,
        {
          method: "POST",
          includeAuth: includeAuth && !guestEmail,
          params,
  ...
  async initiateStripeTransaction(
    txId,
    includeAuth = true,
    guestEmail = "",
    methodType = "card"
    ...
    try {
      ...
      const response = await this.request(
        `/api/payments/transactions/${txId}/initiate-stripe/`,
        {
          method: "POST",
          includeAuth: includeAuth && !guestEmail,
          params,
          body: { method_type: methodType },
        }
      );
      return response.data;
    } catch (error) {
      console.error(
        "initiateStripeTransaction error:",
        error,
        error.response?.data
      );
      return {
